...
Future<List<Monument>> fetchMonuments() async {
  final response = await http
      .get(Uri.parse('http://127.0.0.1:8080/MonumentProjetWS/Monument'));

  if (response.statusCode == 200) {
    // If the server did return a 200 OK response,
    // then parse the JSON.
    List<Monument> objs = jsonDecode(response.body).map<Monument>((roomJson)
    => Monument.fromJson(roomJson)).toList();
